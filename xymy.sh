...
sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=XM9-Sale-$(shell date +%F-%H)-$(VERSION_DIST_SANITIZED)/g' include/image.mk
...
git clone --depth 1 https://github.com/Lienol/openwrt-package package/openwrt-package
...
git clone --depth 1 https://github.com/tty228/luci-app-serverchan package/lean/luci-app-serverchan
...
svn export https://github.com/lllrrr/myok/trunk/curl package/network/utils/curl
...
git clone --depth 1 https://github.com/lllrrr/mysmartdns package/mysmartdns
...
sed -i "s/R8.3.19.0410/XM9.$(date "+%y.%m%d.%H")/g" package/lean/default-settings/files/zzz-default-settings
sed -i "s/R8.3.19.0410/XM9.$(date "+%y.%m%d.%H")/g" files/etc/banner
