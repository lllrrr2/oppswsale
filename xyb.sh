...
sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=XO9-Sale-$(shell date +%F-%H)-$(VERSION_DIST_SANITIZED)/g' include/image.mk
...
sed -i "s/R8.3.19.0410/XO9.$(date "+%y.%m%d.%H")/g" package/lean/default-settings/files/zzz-default-settings
sed -i "s/R8.3.19.0410/XO9.$(date "+%y.%m%d.%H")/g" files/etc/banner
